%% Load .mat File (example: 'PRAC3_Day21_Direct.m') & Start running code on Line 460
% =====
% TIPS
% =====
% - For exploratory analysis using GPFA, we often run only Section 1
%   below, and not Section 2 (which finds the optimal latent
%   dimensionality).  This can provide a substanrtial savings in running
...
[file,path]=uigetfile('*.plx'); % Me: Gets all plx files

datafile = [path,file];
[tscounts, wfcounts, evcounts, slowcounts] = plx_info(datafile,1); %Me: Calls plx_info.m file 
...
%  figure;
%  plot(explained_pca) %Figure 1: This plot will show how much variance is explained by each Principal Component (PC)
%  figure
%  for i =1:length(explained_pca)
%      sumexplained(i) = sum(explained_pca(1:i));
%  end
